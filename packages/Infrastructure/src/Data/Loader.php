...
class Loader
{
    ...
    private function update(InitialData $object)
    {
        ...
        foreach ($metadata->getFieldNames() as $field) {
            $query
                ->set($metadata->getColumnName($field), ':' . $field)
                ->setParameter($field, $this->getFieldValue($object, $field));
        }
    ...
    private function insert(InitialData $object)
    {
        ...
        foreach ($metadata->getFieldNames() as $field) {
            $query
                ->setValue($metadata->getColumnName($field), ':' . $field)
                ->setParameter($field, $this->getFieldValue($object, $field));
        }
        ...
        foreach ($metadata->getAssociationMappings() as $field => $mapping) {
            if ($mapping['type'] == ClassMetadataInfo::MANY_TO_ONE && $mapping['isOwningSide']) {
                $query
                    ->setValue($metadata->getColumnName($field), ':' . $field)
                    ->setParameter($field, $metadata->getFieldValue($object, $field)->getId());
            }
        }
    ...
    private function getFieldValue(InitialData $object, $field)
    {
        $metadata = $this->getMetadata($object);
        $type = Type::getType($metadata->getTypeOfField($field));

        return $type->convertToDatabaseValue(
            $metadata->getFieldValue($object, $field),
    ...
    private function createObject($class, $data)
    {
        switch ($class) {
            case self::CLASS_FIELD:
                $field = Field::createFromJson($data);
                $this->objects[self::CLASS_FIELD][$field->getId()] = $field;
                return $field;
            ...
            case self::CLASS_POLICY:
                ...
                return new Policy(
                    ...
                    $this->getObject(self::CLASS_FIELD, $json['field']),
                    $json['sources'],
                    $data['content']
                );
        };
